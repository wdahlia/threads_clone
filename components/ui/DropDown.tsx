  ...
  return (
    <>
      { screenSize < 670 && popup ?
        ...
        :
        <div className={`absolute z-[999] mobile:-right-[10px] -right-[30px] top-[20px] text-[15px] dark:text-dark-txt text-light-txt font-normal dark:bg-dark-list-bg bg-light-list-bg w-[174px] rounded-[15px] m-[20px] shadow-box dark:border-dark-hr`}>
          <ul>
            { content.flatMap((list, idx) => <li key={nanoid()} className="border-b-light-hr dark:border-b-dark-hr py-[10px] border-b-[1px] last:border-b-0 first:pt-[15px] last:pb-[15px] indent-[20px] cursor-pointer active:dark:bg-dark-bg active:bg-light-bg font-medium text-[15px]">{list}</li>) }
          </ul>
        </div>
      }
    </>
...
// 이중배열 활용, flatMap 사용하여 table/pc 화면에서는 해체하고 mobile의 경우는 threads 레이아웃처럼 구성할 예정
